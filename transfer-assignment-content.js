(async () => {
  ...
  const API = {
    ...
    async get(url) { const r = await fetch(url, { credentials: "same-origin" }); if (!r.ok) { const t = await r.text().catch(()=>""); throw new Error(`GET ${url} failed: ${r.status} ${t.slice(0,500)}`); } return r; },
    ...
    async put(path, body) {
      ...
      if (!r.ok) { const t = await r.text().catch(()=> ""); throw new Error(`PUT ${url} failed: ${r.status} ${t.slice(0,500)}`); }
      return r.json();
    }
  };

  /* ========= UI ========= */
  document.getElementById('wkx-overlay')?.remove();
  document.getElementById('wkx-styles')?.remove();
...
#wkx-modal{background:#fff;max-width:1100px;width:95%;max-height:94vh;border-radius:12px;box-shadow:0 10px 30px rgba(0,0,0,.2);overflow:hidden;font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial;display:flex;flex-direction:column}
#wkx-header{padding:12px 16px;border-bottom:1px solid #eee;display:flex;justify-content:space-between;align-items:center;flex:0 0 auto}
#wkx-body{padding:14px 16px;overflow:auto;flex:1 1 auto;min-height:0}
#wkx-footer{padding:12px 16px;border-top:1px solid #eee;display:flex;gap:8px;justify-content:flex-end;align-items:center;flex:0 0 auto;background:#fff}
#wkx-close{background:none;border:none;font-size:20px;cursor:pointer}
#wkx-spinner{display:none;align-items:center;gap:8px;margin-right:auto}
...
@media(max-width:800px){.wkx-row{grid-template-columns:26px 1fr}.wkx-row>div:nth-child(3),.wkx-row>div:nth-child(4){grid-column:2}}
    `;
    const s = document.createElement("style"); s.id='wkx-styles'; s.textContent = css; document.head.appendChild(s);
  }
  function overlay(html) {
    ...
    const close=()=>{wrap.remove();document.getElementById('wkx-styles')?.remove();};
    wrap.querySelector('#wkx-close').onclick = close;
    wrap.querySelector('#wkx-cancel').onclick = close;
    return wrap;
  }
  function setSpin(on, text) { const sp = document.getElementById('wkx-spinner'); const t = document.getElementById('wkx-progress'); if (!sp||!t) return; if (text) t.textContent = text; sp.classList.toggle('on', !!on); }
  ...
  const DASH_CLASS = '\\-\\u2010\\u2011\\u2012\\u2013\\u2014\\u2212';
  const MIN_AUTO_MATCH_SCORE = 0.70;
  const GENERIC_LAB_RE = new RegExp(`^\\s*(?:preparation\\s*:\\s*)?lab(?:\\s*\\([^)]*\\))?(?:\\s*[:：${DASH_CLASS}]\\s*.*)?\\s*$`,'i');
  ...
  function coreTitle(s, week){
    let t = s || '';
    if (typeof week === 'number') t = stripWeekPrefix(t, week);
    if (GENERIC_LAB_RE.test(t)) return 'Lab';
    return removeTypePrefix(t);
  }
  ...
  const ui = overlay(`
    <div class="wkx-topopts">
      ...
      <small class="mono">Matching treats Lab, Lab (In-person), and Lab with a subtitle as the weekly Lab assignment. Scores below 70% require a manual choice.</small>
    </div>
    ...
    try {
      ...
      for (const pg of weekly){
        ...
        for (const sec of sections){
          ...
          const scored = candidates.map(c=>{
            ...
            const ts = secKey !== '' && secKey === c.key ? 1 : 0;
          ...
          const usable = best.c && best.score >= MIN_AUTO_MATCH_SCORE ? best.c : null;
          const alts = scored.slice(0,3).map(x=>({ id:x.c?.id, name:x.c?.name, url:x.c?.html_url, score:x.score }));
          items.push({ idx: idxCounter++, week: pg.week, pageUrl: pg.url, srcPageTitle: pg.title, sectionHeadingRaw: sec.titleRaw, sectionTitle: sec.titleClean, titleCore, html: sec.html, match: usable ? { id:usable.id, name:usable.name, url:usable.html_url, rest:usable.rest, restCore:usable.restCore } : null, score: best.score, alts });
        }
      }

      function postProcess(items){
        const enforceUnique = byId('opt-unique').checked;
        const byW = new Map(); items.forEach(it=>{ it.conflict=false; it.ambiguous=false; if(!byW.has(it.week)) byW.set(it.week, []); byW.get(it.week).push(it); });
        for (const arr of byW.values()){ arr.sort((a,b)=>b.score-a.score); const used = new Set(); for (const it of arr){ const rival = it.alts[1]?.score || 0; it.ambiguous = !!it.match && (it.score - rival) < 0.02 && rival >= 0.84; if (enforceUnique && it.match){ if (used.has(it.match.id)) { it.conflict = true; } else { used.add(it.match.id); } } } }
      }
      postProcess(items);
      ...
      const weeksHtml = [...grouped.entries()].sort((a,b)=>a[0]-b[0]).map(([wk, arr])=>{
        const rows = arr.map(it=>{
          ...
          let note = '';
          if (!it.match){ cls='err'; note='<div class="wkx-note">No automatic match met the 70% minimum. Use Change to select one.</div>'; }
          else if (it.conflict){ cls = 'err'; note = '<div class="wkx-note">Conflict: another section already claimed this assignment. Choose a different one.</div>'; }
          else if (it.ambiguous){ cls = 'warn'; const alt = it.alts[1]; note = `<div class="wkx-note">Ambiguous: also close match "${alt?.name || 'Unknown'}" at ${Math.round((alt?.score||0)*100)}%.</div>`; }
          const matchHtml = it.match ? `<a href="${it.match.url}" target="_blank" rel="noopener">${it.match.name}</a><div><small class="mono">match on "${it.match.restCore || it.match.rest}"</small></div>` : `<span>No same-week automatic match</span>`;
          ...
          return `
            <div class="wkx-row" data-idx="${it.idx}">
              ...
              <div class="wkx-matchbox">
                ...
                <div class="wkx-override ${!it.match||it.conflict||it.ambiguous?'on':''}">
                  <div class="row">
                    ...
                    <button type="button" class="Button Button--small Button--secondary wkx-cancel-override">Cancel</button>
                  </div>
        ...
        return `<div class="wkx-week" data-week="${wk}"><div class="head"><div><strong>Week ${wk}</strong> <small class="mono">- ${arr.length} sections</small></div><div><label style="display:inline-flex;align-items:center;gap:6px;cursor:pointer"><input type="checkbox" class="wkx-week-all" data-week="${wk}" checked> Select all good matches</label></div></div><div class="rows">${rows || "<div style='padding:6px'>No sections</div>"}</div></div>`;
      }).join('');

      setSpin(false);
      byId('wkx-preview').innerHTML = `<div style="margin-bottom:6px"><ol style="margin:6px 0 8px;padding-left:18px"><li>Find all pages titled "Week X: Assignments".</li><li>Parse each page into per-assignment sections for that week only.</li><li>Match each section to assignments named "Week X - Title". Use Change to override.</li><li>Start to write descriptions and insert link lines on weekly pages.</li></ol><small class="mono">Generic lab headings now match Lab. Ambiguous matches are warned and unchecked. Low scores are not assigned automatically.</small></div>${weeksHtml}`;
      ...
      document.querySelectorAll('.wkx-row').forEach(rowEl=>{
        const changeBtn=rowEl.querySelector('.wkx-change'); const panel=rowEl.querySelector('.wkx-override'); const applyBtn=rowEl.querySelector('.wkx-apply'); const cancelBtn=rowEl.querySelector('.wkx-cancel-override'); const select=rowEl.querySelector('.wkx-select');
        ...
        applyBtn.addEventListener('click',()=>{
          ...
          item.match = { id:chosen.id, name:chosen.name, url:chosen.url||chosen.html_url, rest:chosen.rest||chosen.name, restCore };
      ...
      console.error(e); setSpin(false); byId('wkx-preview').innerHTML=`<div class="ic-flash-error"><strong>Could not build preview.</strong><pre style="white-space:pre-wrap">${e.message}</pre></div>`;
    }
  }
  ...
  async function runUpdates(){
    ...
    if(selected.some(s=>!s.match)){ alert('One or more selected rows do not have an assignment match. Choose a match or uncheck those rows.'); return; }
    const summary = selected.map(s=>`• Week ${s.week}: ${s.sectionTitle} → ${s.match.name}`).join('\n'); if(!confirm(`Proceed to update ${selected.length} assignments?\n\n${summary}`)) return;

    setSpin(true,'Updating assignments...'); const results=[]; let i=0;
    for (const s of selected){ i++; try {
    ...
    const plainPageFailed = failedPages.map(p=>`${p.pageUrl} - ${p.err||'Unknown error'}`).join('\n');
      ...
      `<div style="margin-top:10px">${htmlList}<div style="margin-top:12px;display:flex;gap:8px"><button class="Button Button--secondary" id="copy-html">Copy HTML summary</button><button class="Button Button--secondary" id="copy-text">Copy plain text</button></div></div>`
    );
